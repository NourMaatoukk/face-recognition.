...
if (!file.exists("face_encodings.pkl")) {
  cat("❌ Face encodings not found. Run setup.R first.\n")
...
def log_attendance(name, similarity, action):
    ...
    if action == "JOIN":
        ...
        print(f"🟢 {name} JOINED at {time_str}")
        ...
        duration_str = f"{hours}h {minutes}m" if hours > 0 else f"{minutes}m"
        time_str = timestamp.strftime("%H:%M:%S")
        print(f"🔴 {name} LEFT at {time_str} (Duration: {duration_str})")
...
for (path in conda_paths) {
  ...
  if (!is.null(test_result) && length(test_result) > 0) {
    conda_cmd <- path
    cat("✅ Conda detected at:", conda_cmd, "\n")
    break
  }
}

if (is.null(conda_cmd)) {
  cat("❌ Conda not found. Trying direct Python...\n")
